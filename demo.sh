#!/bin/bash -eu
# This is a demonstration for the use of the pplacer suite of programs.
# It covers the use of placement, visualization, classification, and comparison.
...
# and that you have java installed. The software is only compiled for OS X and
# linux. A reasonably recent laptop with 2GB of memory should be able to run 
# this code just fine.

# For the purposes of this demo, we start with a little bash function `aptx` to
...
# of pplacer.
pplacer -c vaginal_16s.refpkg -r data/refalign.p4z1r36.fasta data/p4z1r36.fasta
...
mokaphy kr example_place_files/*
read
...
mokaphy pca -o pca -c vaginal_16s.refpkg example_place_files/*
cat pca.trans
aptx pca.xml
...
mokaphy cluster -o my_cluster example_place_files/*
aptx my_cluster/mass_trees/0006.phy.fat.xml 
aptx my_cluster/cluster.tre 

# There is also a way using mokaphy to collect all of these together into named
# units to ease visualization of 
aptx clusters_0121.xml
